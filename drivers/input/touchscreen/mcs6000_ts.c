...
#define TOUCH_HOME    	249
#define TOUCH_MENU	250
...
extern int esd_recovery_disable;
...
void mcs6000_firmware_info(unsigned char* fw_ver, unsigned char* hw_ver)
{
	...
	*hw_ver = data;
	...
	if (*fw_ver > MCS6000_TS_MAX_FW_VERSION ||
		*hw_ver > MCS6000_TS_MAX_HW_VERSION) {
		printk(KERN_INFO "LCD is not connected!Do not run esd recovery\n");
		esd_recovery_disable = 1;
	} else {
		esd_recovery_disable = 0;
	}
#endif
}
